...
import com.webank.weid.kit.amop.base.*;
import com.webank.weid.kit.amop.request.*;
import com.webank.weid.kit.amop.callback.CommonCallbackWeId;
import com.webank.weid.kit.amop.callback.KeyManagerCallbackWeId;
...
import com.webank.weid.kit.protocol.response.*;
import com.webank.weid.kit.amop.callback.OnNotifyCallbackV2;
import com.webank.weid.kit.amop.callback.OnNotifyCallbackV3;
...
public class AmopServiceImpl implements AmopService {
    ...
    public ResponseData<PolicyAndChallenge> getPolicyAndChallenge(
        ...
        try {
            if (StringUtils.isBlank(this.amopId)) {
                ...
                return new ResponseData<PolicyAndChallenge>(null, KitErrorCode.ILLEGAL_INPUT);
            }
            ...
            if (retResponse.getErrorCode().intValue() != KitErrorCode.SUCCESS.getCode()) {
                ...
                return new ResponseData<PolicyAndChallenge>(
                        null,
                        KitErrorCode.getTypeByErrorCode(retResponse.getErrorCode().intValue())
                );
            }
            ...
            return new ResponseData<PolicyAndChallenge>(result.getPolicyAndChallenge(), kitErrorCode);
        } catch (Exception e) {
            logger.error("getPresentationPolicy failed due to system error. ", e);
            return new ResponseData<PolicyAndChallenge>(null, KitErrorCode.UNKNOW_ERROR);
        }
    }
    ...
    public ResponseData<RequestIssueCredentialResponse> requestIssueCredential(
        ...
        if (errCode != KitErrorCode.SUCCESS.getCode()) {
            logger.error(
                    "[requestIssueCredential] prepareZkpCredential failed. error code :{}",
                    errCode);
            return new ResponseData<RequestIssueCredentialResponse>(null,
                    KitErrorCode.getTypeByErrorCode(errCode));
        }
        ...
        if (errorCode != KitErrorCode.SUCCESS.getCode()) {
            logger.error(
                    ...
                    errorCode);
            return new ResponseData<RequestIssueCredentialResponse>(null,
                    KitErrorCode.getTypeByErrorCode(errorCode));
        }
    ...
    private KitErrorCode checkIssueCredentialArgs(RequestIssueCredentialArgs args) {
        ...
        if (args == null
            ...
            return KitErrorCode.ILLEGAL_INPUT;
        ...
        if (!WeIdUtils.isWeIdValid(auth.getWeId())) {
            return KitErrorCode.WEID_INVALID;
        }
    ...
    private ResponseData<PresentationE> preparePresentation(
        ...
        return new ResponseData<PresentationE>(presentationResp.getResult(), KitErrorCode.getTypeByErrorCode(presentationResp.getErrorCode()));
    }
    ...
    private void blindCredentialSignature(RequestIssueCredentialResponse response, String userId) {
        ...
        if (dbResponse.getErrorCode().intValue() != KitErrorCode.SUCCESS.getCode()) {
            throw new DatabaseException("database error!");
        }
    }
    ...
    protected <T, F extends AmopBaseMsgArgs> ResponseData<T> getImpl(
        ...
        com.webank.weid.blockchain.protocol.amop.AmopCommonArgs amopCommonArgs = new com.webank.weid.blockchain.protocol.amop.AmopCommonArgs();
        ...
        if (null == msgBodyObj) {
            responseStruct.setErrorCode(KitErrorCode.UNKNOW_ERROR);
        }
