...
public class PdfTransportationImpl
    ...
    public <T extends JsonSerializer> ResponseData<byte[]> serialize(
        ...
        try {
            ...
            return new ResponseData<>(pdfFileByte, KitErrorCode.SUCCESS);
        } catch (WeIdBaseException e) {
            logger.error("[serialize] PdfTransportation serialization due to base error.", e);
            return new ResponseData<>(null, KitErrorCode.getTypeByErrorCode(e.getErrorCode().getCode()));
        } catch (Exception e) {
            logger.error("[serialize] PdfTransportation serialization due to unknown error.", e);
            return new ResponseData<>(null, KitErrorCode.TRANSPORTATION_BASE_ERROR);
        }
    }
    ...
    public <T extends JsonSerializer> ResponseData<Boolean> serialize(
        ...
        try {
            if (outputPdfFilePath == null || outputPdfFilePath.length() == 0) {
                ...
                return new ResponseData<>(false, KitErrorCode.ILLEGAL_INPUT);
            }
            ...
            if (res.getResult() == null) {
                ...
                return new ResponseData<>(
                    false, KitErrorCode.getTypeByErrorCode(res.getErrorCode()));
            } else {
                //生成文件
            ...
            return new ResponseData<>(true, KitErrorCode.SUCCESS);
        } catch (WeIdBaseException e) {
            logger.error("[serialize] PdfTransportation serialization due to base error.", e);
            return new ResponseData<>(false, KitErrorCode.getTypeByErrorCode(e.getErrorCode().getCode()));
        } catch (Exception e) {
            logger.error("[serialize] PdfTransportation serialization due to unknown error.", e);
            return new ResponseData<>(false, KitErrorCode.TRANSPORTATION_BASE_ERROR);
        }
    }
        ...
        try {
            ...
            if (file.exists() && !file.isDirectory()) {
                try {
                    ...
                    logger.error("pdf template load error:{}.", KitErrorCode.BASE_ERROR, e);
                    return new ResponseData<>(null,  KitErrorCode.BASE_ERROR);
                }
            } else {
                logger.error("pdf template directory illegal:{}.", KitErrorCode.ILLEGAL_INPUT);
                return new ResponseData<>(null,  KitErrorCode.ILLEGAL_INPUT);
            }
    ...
    public <T extends JsonSerializer> ResponseData<T> deserialize(
        ...
        KitErrorCode errorCode = checkWeIdAuthentication(weIdAuthentication);
        if (errorCode != KitErrorCode.SUCCESS) {
        ...
        try {
            ...
            if (encodeType == null) {
                return new ResponseData<>(null, KitErrorCode.TRANSPORTATION_PROTOCOL_ENCODE_ERROR);
            }
            ...
            if (!pdfAttributeInfo.getTemplateId().equals("null")
                ...
                return new ResponseData<>((T)presentationE, KitErrorCode.SUCCESS);
            } else {
                logger.info("PdfTransportationImpl deserialization finished.");
                return new ResponseData<>(object, KitErrorCode.SUCCESS);
            }
        } catch (WeIdBaseException e) {
    ...
    private  <T extends JsonSerializer> byte[] assemblePdf(
        ...
        try {
            //检查presentation完整性
            KitErrorCode errorCode = checkProtocolData(object);
            if (errorCode != KitErrorCode.SUCCESS) {
    ...
    private <T extends JsonSerializer> ResponseData<byte[]> checkPara(
        ...
        //检查协议配置完整性
        KitErrorCode errorCode = checkEncodeProperty(property);
        if (errorCode != KitErrorCode.SUCCESS) {
            logger.error("checkEncodeProperty fail, errorCode:{}.", errorCode);
            return new ResponseData<>(null, KitErrorCode.getTypeByErrorCode(errorCode.getCode()));
        }
